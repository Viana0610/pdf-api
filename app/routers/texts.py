...
# Lista todos os textos salvos (limitado a 100 registros)
@router.get("/", response_model=List[ExtractedTextResponse])
def list_texts(
...
# Busca um texto específico pelo ID
@router.get("/{text_id}", response_model=ExtractedTextResponse)
def get_text(
    text_id: int,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user)  # Exige autenticação
...
# Atualiza parcialmente um texto existente
@router.put("/{text_id}", response_model=ExtractedTextResponse)
def update_text(
    text_id: int,
...
# Remove um texto do banco pelo ID
