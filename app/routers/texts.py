...
# Router responsável pelos endpoints de CRUD dos textos extraídos
...
# Endpoint: GET /texts/
# Lista todos os textos extraídos que estão salvos no banco de dados
# Retorna uma lista de registros com id, filename, content e datas
# Limitado a 5 registros por requisicao para nao sobrecarregar a API
...
def list_texts(
    ...
    current_user=Depends(get_current_user)  # Exige autenticação via token JWT
):
    # Busca todos os registros da tabela limitando a 5 resultados
    return db.query(ExtractedText).limit(5).all()


# Endpoint: GET /texts/{text_id}
# Busca um único registro pelo ID informado na URL
# Se o ID existir, retorna o registro completo com todos os campos
# Se o ID não existir, retorna erro 404 — registro nao encontrado
# Requer autenticação via token JWT
@router.get(
    "/{text_id}",
...
def get_text(
    ...
    if not text:
        # Registro não encontrado — retorna erro 404
...
# Aceita filename, content ou ambos — só atualiza o que for enviado
# Os campos não enviados permanecem com o valor original no banco
# Se o ID não existir, retorna erro 404 — registro não encontrado
# Requer autenticação via token JWT
@router.put(
    "/{text_id}",
    response_model=ExtractedTextResponse,
...
def update_text(
    ...
    # Verifica cada campo — só atualiza se foi enviado na requisição
    # Se o campo for None significa que nao foi enviado, então mantem o valor original
...
# A operação nao tem volta — o registro e deletado definitivamente
# Não remove o arquivo PDF original, apenas o registro do texto extraído
# Se o ID nao existir, retorna erro 404 — registro não encontrado
# Requer autenticação via token JWT
@router.delete(
    "/{text_id}",
...
def delete_text(
    ...
    db.delete(text)
    # Confirma a operação — sem o commit o delete não e salvo
    db.commit()
